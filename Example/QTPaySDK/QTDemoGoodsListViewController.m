...
- (void)viewDidLoad {
    ...
    [QTPaySDK setQTPayWithAppCode:QTSDKAppCode appKey:QTSDKAppKey accessToken:[QTDemoUserInfo sharedInstance].token appScheme:@"QTDemo"];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if(indexPath.row >= 3) {
        ...
        [manager GET:[NSString stringWithFormat:@"%@/ordertoken?total_amt=%@&out_mchnt=%@",QTSDKDemoBaseAPI,self.order.total_amt,QTSDKDemoOutMerchant] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
            [MBProgressHUD hideHUDForView:weakSelf.view animated:NO];
            weakSelf.order.order_token = responseObject[@"token"];
            QTLog(@"token -- %@", weakSelf.order.order_token);
